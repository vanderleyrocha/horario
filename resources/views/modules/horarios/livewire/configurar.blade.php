...
<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="bg-white shadow sm:rounded-lg p-6">
        ...
        <div>
            ...
            @if ($etapaAtual === 1)
                @include('modules.horarios.livewire.partials.etapa-configuracao-basica')
            @endif
            ...
            @if ($etapaAtual === 4)
                @include('modules.horarios.livewire.partials.etapa-configuracao-algoritmo-genetico')
            @endif

            {{-- Etapa 5: Resumo e Geração (antiga Etapa 4) --}}
            @if ($etapaAtual === 5)
                @php
                    Illuminate\Support\Facades\Log::info("Renderizando etapa resumo e geração para o horário ID: {$horario->id} via modules.horarios.livewire.configurar");
                @endphp
