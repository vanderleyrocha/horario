<div>
    <div class="max-w-3xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('turmas.index') }}" wire:navigate 
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Editar Turma</h2>
                    <p class="text-gray-600 mt-1">Atualize as informações da turma</p>
                </div>
                <button 
                    wire:click="delete"
                    wire:confirm="Tem certeza que deseja excluir esta turma? Esta ação não pode ser desfeita."
        ...
        <form wire:submit="update" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-700">Status da Turma</p>
                        <p class="text-xs text-gray-500 mt-1">Ative ou desative a turma no sistema</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" wire:model="ativa" class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span class="ms-3 text-sm font-medium" :class="$wire.ativa ? 'text-green-600' : 'text-red-600'">
                            {{ $ativa ? 'Ativa' : 'Inativa' }}
                        </span>
                    </label>
                </div>
            ...
            <div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all {{ $turno === 'matutino' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200 hover:border-gray-300' }}">
                            <input 
                                type="radio" 
                                wire:model="turno"
                                value="matutino"
                        ...
                        <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all {{ $turno === 'vespertino' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300' }}">
                            <input 
                                type="radio" 
                                wire:model="turno"
                                value="vespertino"
                        ...
                        <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all {{ $turno === 'noturno' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300' }}">
                            <input 
                                type="radio" 
                                wire:model="turno"
                                value="noturno"
                ...
                <div class="flex">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">
                            Cadastrado em: {{ $turma->created_at->format('d/m/Y H:i') }}<br>
                            Última atualização: {{ $turma->updated_at?->format('d/m/Y H:i') }}
                        </p>
                    </div>
                </div>
            </div>
