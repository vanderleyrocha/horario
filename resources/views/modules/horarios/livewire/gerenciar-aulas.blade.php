<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <button wire:click="abrirModal" type="button" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center space-x-2 transition-colors">
    ...
    <div class="bg-gray-50 rounded-lg p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            {{-- Busca --}}
            <div>
                <input type="text" wire:model.live="busca" placeholder="Buscar por professor, disciplina ou turma..."
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>

            {{-- Filtro Turma --}}
            <div>
                <select wire:model.live="filtroTurma" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            {{-- Filtro Professor --}}
            <div>
                <select wire:model.live="filtroProfessor" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center space-x-3 mb-2">
                                <div class="w-4 h-4 rounded" style="background-color: {{ $aula->disciplina->cor ?? '#6B7280' }}">
                                </div>
                        ...
                        <div class="flex items-center space-x-2 ml-4">
                            <button wire:click="duplicar({{ $aula->id }})" class="p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Duplicar"
                                type="button">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button wire:click="editar({{ $aula->id }})" class="p-2 text-gray-600 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Editar"
                                type="button">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button wire:click="excluir({{ $aula->id }})" wire:confirm="Tem certeza que deseja excluir esta aula?"
                                class="p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Excluir" type="button">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if ($horario->configuracaoHorario)
                    ...
                    <div class="mt-4">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-500 {{ $percentualOcupacao > 90 ? 'bg-red-500' : ($percentualOcupacao > 75 ? 'bg-yellow-500' : 'bg-green-500') }}"
    ...
    @else
        ...
        <div class="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            ...
            <button wire:click="abrirModal" type="button" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
    ...
    @if ($modalAberto)
        <div wire:key="modal-{{ $editandoId ? 'edit-' . $editandoId : 'create' }}">
            @include('modules.horarios.livewire.partials.modal-aula')
        </div>
    @endif
</div>
