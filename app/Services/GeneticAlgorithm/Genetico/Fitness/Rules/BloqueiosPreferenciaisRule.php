...
class BloqueiosPreferenciaisRule implements FitnessRuleInterface, SoftRuleInterface
{
            ...
            if ($gene->professor) {
                ...
                if ($this->restricoes->has($key)) {
                    foreach ($this->restricoes[$key] as $restricao) {
                        if ($restricao->dia_semana == $gene->diaSemana && $restricao->tempo == $gene->periodoDia && $restricao->tipo == 'preferencial') {
                            $penalidade += 0.5; // Penalidade menor para bloqueio preferencial
                            $conflicts[] = "Professor {$gene->professor->nome} alocado em horário preferencialmente bloqueado (Dia {$gene->diaSemana}, Período {$gene->periodoDia}).";
                        }
                    }
                }
            }
            ...
            if ($gene->turma) {
                ...
                if ($this->restricoes->has($key)) {
                    foreach ($this->restricoes[$key] as $restricao) {
                        if ($restricao->dia_semana == $gene->diaSemana && $restricao->tempo == $gene->periodoDia && $restricao->tipo == 'preferencial') {
                            $penalidade += 0.5; // Penalidade menor para bloqueio preferencial
                            $conflicts[] = "Turma {$gene->turma->nome} alocada em horário preferencialmente bloqueado (Dia {$gene->diaSemana}, Período {$gene->periodoDia}).";
                        }
                    }
                }
            }
    ...
    {
        return 'Bloqueios Preferenciais';
    }
}
